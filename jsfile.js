require("dotenv").config({path: "private.env"}); //we add this for importing the variables from private.env file and store them in process.env
const mongoose = require("mongoose"); //importing mongoose library
const uri = process.env.Mongo_uri; //we can take our data from process.env
    mongoose.connect(uri) //connecting mongodb db to nodejs
    .then(()=>console.log("connection done"))
    .catch((error)=>console.error("there is some issues in connection ",error))
const {Schema} = mongoose; //destructing the Schema property from mongoose library 
my_schema = new Schema({ //defining new schema
    age : {type : Number, required : true},
    name : {type : String, required : true, unique : true},
    favoritfood : [String]
})
const person = new mongoose.model("Person",my_schema); //create the collection in the db if there is no collection named people if there is it won't create a new it will work with the existing one 
async function asyncfunc(){
    await person.deleteMany({})
    const person1 = new person({age:19,name: "ilyasse",favoritfood:["pizza","tacos","cosamia"]});
    try {
        await person1.save();  
        console.log("saved successfully")
    }
    catch(err){
        console.error("error in saving :",err)
    }
    await person.create([
        {name:"hanane",age:42,favoritfood:["cosamia","tacos","sefa"]},
        {name:"hajjaj",age:50,favoritfood:["kssksso","reffisa","tajine","tanjiya"]},
        {name:"reda",age:15,favoritfood:["tacos","pizza","tanjiya"]},
        {name:"saif",age:5,favoritfood:["pizza","tacos","cosamia"]},
        {name:"wissam",age:17,favoritfood:["tacos","tacos","cosamia"]},
        { name: "ali", age: 28, favoritfood: ["burritos", "pizza", "salad"] },
        { name: "sara", age: 34, favoritfood: ["burritos", "tacos", "tajine"] },
        { name: "omar", age: 22, favoritfood: ["burritos", "pasta", "tanjiya"] },
        { name: "laila", age: 19, favoritfood: ["burritos", "cosamia", "tacos"] },
        { name: "youssef", age: 40, favoritfood: ["burritos", "pizza", "tacos"] },
        { name: "nour", age: 25, favoritfood: ["burritos", "tajine", "sefa"] }
    ])
    console.log("finding the first person who like tanjiya ")
    const ilyasse = await person.findOne({favoritfood : {$in : ["tanjiya"]} });
    console.log(ilyasse);
}
asyncfunc();
async function findingbyid(id){
    const doc = person.findById(id);
    if (!doc){
        console.log("the document is not found");
    }else{
        console.log("the original document is:",doc);
        await doc.favoritfood.push("hamburger");
        await doc.save();
        console.log("the modified is:",doc)
    }
}
findingbyid('691b6103d6df7db6b373a41a');
async function findbyname(Name){
    const doc = await person.findOneAndUpdate({name:Name},{$set : {age : 20 }},{new:true});
    console.log("the new doc is :",doc);
    
}
async function findandrem(id) {
    try{
        await person.findByIdAndRemove(id)
        console.log(`the doc with id = ${id} is removed`)
    }catch(err){
        console.error("the error is:",err)
    }
}
async function chainofqueries() {
    const data = person.find({favoritfood : {$in : ["burritos"]}}).sort({name:1}).limit(3).select("-age").exec((err,data)=>{
        if(err) return console.error(err);
        console.log(data);
    })
}